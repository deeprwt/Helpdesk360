import { TicketApproval } from '../types';
...
const EngineerApprovals = () => {
  ...
  const pending = approvals.filter((approval: TicketApproval) => approval.state === 'pending');
  const history = approvals.filter((approval: TicketApproval) => approval.state !== 'pending').slice(0, 5);

  // ✅ Typed placeholder to match TicketApproval
  const placeholder: TicketApproval[] =
    ...
    >
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '24px' }}>
        <section className="glass-panel" style={{ padding: '24px' }}>
          ...
          >
            <div>
              <p className="eyebrow" style={{ margin: 0, color: 'var(--ink-400)' }}>
                Queue
              </p>
            ...
            <button
              type="button"
              style={{ border: 'none', background: 'transparent', color: 'var(--accent-blue-strong)', fontWeight: 600 }}
            >
              View history
            </button>
          </div>
              ...
              >
                <div>
                  ...
                  <p style={{ margin: '4px 0 0', fontWeight: 600 }}>
                    {entry.ticketTitle ?? 'Loading approval'}
                  </p>
                  <p style={{ margin: 0, color: 'var(--ink-500)', fontSize: '14px' }}>
                    {entry.note ?? 'Awaiting note'}
                  </p>
                </div>
                ...
                <div>
                  ...
                  <p style={{ margin: 0, color: '#ff6b6b', fontWeight: 600 }}>
          ...
          <div className="glass-panel" style={{ padding: '20px' }}>
            ...
            <ul style={{ listStyle: 'none', padding: 0, display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {(history.length ? history : placeholder).map((row) => (
                <li
                  key={row.id}
                  style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px', fontWeight: 600 }}
                >
                  <span style={{ color: 'var(--ink-500)' }}>{formatTime(row.createdAt)}</span>
                  <span style={{ flex: 1, marginLeft: '12px' }}>
                    {row.ticketTitle ?? 'Approval update'}
                  </span>
                  <span
                    style={{
          ...
          <div className="glass-panel" style={{ padding: '20px' }}>
            ...
            <ul style={{ listStyle: 'none', padding: 0, display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {defaultPolicies.map((policy) => (
                <li
                  key={policy.title}
                  style={{ border: '1px solid var(--border-strong)', borderRadius: '16px', padding: '14px' }}
                >
